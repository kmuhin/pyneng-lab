...
def parse_cdp_neighbors(command_output):
    ...
    Функция возвращает такой словарь:
    {("R4", "Fa0/1"): ("R5", "Fa0/1"),
     ("R4", "Fa0/2"): ("R6", "Fa0/0")}
    """
    device = ''
    ...
    for line in command_output.split('\n'):
        ...
        if not line:
            continue
        # split and use col1 to detect certain lines
        col1, *line_splitted = line.split()
        if not device and '>' in col1:
            device, _ = col1.split('>')
            continue
        # continue until I find device
        if not device:
            continue
        # if the line starts with 'Device' then the next lines contain info about neighbors.
        # so I set bln_neighobrs = True
        if 'Device' in col1:
            bln_neighbors = True
            continue
        if bln_neighbors:
            neighbor = col1
            intf_local = line_splitted[0] + line_splitted[1]
            intf_remote = line_splitted[-2] + line_splitted[-1]
            dev_neighbors[device, intf_local] = neighbor, intf_remote
    return dev_neighbors
