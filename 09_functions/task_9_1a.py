...
def generate_access_config(intf_vlan_mapping, access_template, psecurity=None):
    ...
    psecurity - дополнительный список команд

    Возвращает список команд для конфигурации всех портов в режиме access на основе шаблона access_template
    вклая команды списка psecurity, если был указан.
    """
    list_config_interfaces = []
    for intf, vlan in intf_vlan_mapping.items():
        list_config_interfaces.append(f'interface {intf}') 
        for cmd in access_template:
            if cmd.endswith('vlan'):
                cmd = f'{cmd} {vlan}'
            list_config_interfaces.append(cmd)
        if psecurity:
            list_config_interfaces.extend(psecurity)
    return list_config_interfaces 
